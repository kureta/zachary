from zachary.preprocess.base import BaseDataset, Configuration, BaseMidiDataset
...
class AtemporalMidiDataset(BaseMidiDataset):
    def __init__(self, conf=Configuration()):
        super(AtemporalMidiDataset, self).__init__(conf=conf)
...
class TemporalMidiDataset(BaseMidiDataset):
    def __init__(self, conf=Configuration(), example_length=64, example_hop_length=32):
        super(TemporalMidiDataset, self).__init__(conf=conf)
    ...
    def update_strided(self):
        stride = self.midi.stride()
        shape = self.midi.shape
        n_examples = (shape[0] - self.example_length) // self.example_hop_length + 1
        self.strided_examples = self.midi.as_strided(
            (n_examples, shape[1], self.example_length),
            (stride[0] * self.example_hop_length, stride[1], stride[0]))
